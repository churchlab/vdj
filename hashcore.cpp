 ...
 */
/*
char* FeatureSet::getName() {
    return string(_name);
}

char* FeatureSet::getSequence() {
...
ReferenceSet::ReferenceSet(list<ObservationSet*> observations ){
    ...
    // NOTE pseudocounts are accomodated for in the odds calculation
    //addPseudocounts();
...
/*
bool ReferenceSet::addPseudocounts(){
    
    // check to see if pseudocounts have already been added
    if( _pseudocounts ){
        return false;
    }

    map<unsigned long, int>::iterator o_itr;

    for( o_itr = _refs->begin(); o_itr != _refs->end() ; o_itr++ ){
       (*o_itr).second += 1; 
    }
    _pseudocounts = true;
    return true;
...
ObservationSet::ObservationSet(char *name, char *sequence){
    int nm_len = strlen(name);
    int seq_len = strlen(sequence);

    _name = (char*) malloc(nm_len + 1);
    _sequence = (char*) malloc(seq_len + 1);
    strcpy(_name, name);
    strcpy(_sequence, sequence);
    _obsset = extractFeatures(_sequence, new map<unsigned long, int>());
   // printf("Observation Set size: %d\n", _obsset->size());
}
ObservationSet::ObservationSet(string name, string sequence){
    ObservationSet( name.c_str(), sequence.c_str() );
    return;
}
