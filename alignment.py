...
# like v1.1 aligner except uses the new hasher

import alignmentcore

class vdj_aligner_v11_new_hash(object):
    ...
    def __init__(self, verbose=False):
        ...
        # load reference sequences
        vdj_aligner_v11_new_hash.load_references(self.Vhasher,refseq.IGHV_seqs)
        vdj_aligner_v11_new_hash.load_references(self.Dhasher,refseq.IGHD_seqs)
        vdj_aligner_v11_new_hash.load_references(self.Jhasher,refseq.IGHJ_seqs)
    ...
    def align_chain(self,chain,verbose=False):
        ...
        # queryannot,querykeys = vdj_aligner_v11_new_hash.seq2kmerannot(query,self.seedpatterns)
        ...
        # goodVseglist = [ seg[0] for seg in vdj_aligner_v11_new_hash.dict2sorteddecreasingitemlist(Vscores_hash,'value')[0:self.numCrudeVCandidates] ]
        # 
        # if verbose:
        #     print goodVseglist
        # 
        ...
        goodVseglist = self.hash_V(query,verbose)[:self.num_V_candidates]
        ...
        for goodVseg in goodVseglist:
            ...
            currscore = vdj_aligner_v11_new_hash.scoreVJalign(scores)
        ...
        if bestVseg != '':
            Valnref,Valnrefcoords,Valnquery,Valnquerycoords = vdj_aligner_v11_new_hash.construct_alignment( refseq.IGHV_seqs[bestVseg], query, bestVscoremat, bestVtracemat )
            query,v_end_idx = vdj_aligner_v11_new_hash.pruneVregion( Valnref, Valnrefcoords, Valnquery, Valnquerycoords, bestVseg, query )
            chain.add_tags('v_end_idx|%d'%v_end_idx)
        ...
        # goodJseglist = [ seg[0] for seg in vdj_aligner_v11_new_hash.dict2sorteddecreasingitemlist(Jscores_hash,'value')[0:self.numCrudeJCandidates] ]
        # 
        # if verbose:
        #     print goodJseglist
        # 
        ...
        goodJseglist = self.hash_J(query,verbose)[:self.num_J_candidates]
        ...
        for goodJseg in goodJseglist:
            ...
            # pure python:
            #scores,trace = vdj_aligner_v11_new_hash.alignNW( refseq.IGHJ_seqs[goodJseg], query )
        ...
        if bestJseg != '':
            Jalnref,Jalnrefcoords,Jalnquery,Jalnquerycoords = vdj_aligner_v11_new_hash.construct_alignment( refseq.IGHJ_seqs[bestJseg], query, bestJscoremat, bestJtracemat )
            (query,j_start_idx) = vdj_aligner_v11_new_hash.pruneJregion( Jalnref, Jalnrefcoords, Jalnquery, Jalnquerycoords, bestJseg, query )
        ...
        #     queryannot,querykeys = vdj_aligner_v11_new_hash.seq2kmerannot(query,self.miniseedpatterns)
        #     
        #     t9 = time.time()
        #     
        #     Dscores_hash = {}
        ...
        #     goodDseglist = [ seg[0] for seg in vdj_aligner_v11_new_hash.dict2sorteddecreasingitemlist(Dscores_hash,'value')[0:self.numCrudeDCandidates] ]
        #     
        #     if verbose:
        #         print goodDseglist
        #     
        ...
        #         
        #         # pure python:
        #         #scores,trace = vdj_aligner_v11_new_hash.alignSW( refseq.IGHD_seqs[goodDseg], query )
        #         
        #         currscore = vdj_aligner_v11_new_hash.scoreDalign(scores)
    ...
    def seq2posstrand(self,seq):        
        seqannot,seqkeys = vdj_aligner_v11_new_hash.seq2kmerannot(seq,[self.patternPos])
    ...
    def seqdict2kmerannot(seqdict,patterns):
        ...
        for seq in seqdict.iteritems():
            seqannot,seqkeys = vdj_aligner_v11_new_hash.seq2kmerannot(seq[1],patterns)
            seqlistannot[seq[0]] = {}
            seqlistkeys[seq[0]]  = {}
            for pattern in patterns:
...
# this is the v1.1 aligner

class vdj_aligner_v11(object):
    ...
    def __init__(self, verbose=False):
        ...
        self.Vseqlistannot,self.Vseqlistkeys = vdj_aligner_v11.seqdict2kmerannot( refseq.IGHV_seqs, self.seedpatterns )
        self.Dseqlistannotmini,self.Dseqlistkeysmini = vdj_aligner_v11.seqdict2kmerannot( refseq.IGHD_seqs, self.miniseedpatterns )
        self.Jseqlistannot,self.Jseqlistkeys = vdj_aligner_v11.seqdict2kmerannot( refseq.IGHJ_seqs, self.seedpatterns )
        
        # Generate reference data for positive sequence ID
        posVseqlistannot,posVseqlistkeys = vdj_aligner_v11.seqdict2kmerannot( refseq.IGHV_seqs, [self.patternPos] )
        posJseqlistannot,posJseqlistkeys = vdj_aligner_v11.seqdict2kmerannot( refseq.IGHJ_seqs, [self.patternPos] )
        negVseqlistannot,negVseqlistkeys = vdj_aligner_v11.seqdict2kmerannot( vdj_aligner_v11.seqdict2revcompseqdict(refseq.IGHV_seqs), [self.patternPos] )
        negJseqlistannot,negJseqlistkeys = vdj_aligner_v11.seqdict2kmerannot( vdj_aligner_v11.seqdict2revcompseqdict(refseq.IGHJ_seqs), [self.patternPos] )
    ...
    def align_chain(self,chain,verbose=False):
        ...
        # compute hashes from query seq
        queryannot,querykeys = vdj_aligner_v11.seq2kmerannot(query,self.seedpatterns)
        ...
        # get numCrudeVCandidates highest scores in Vscores and store their names in descending order
        goodVseglist = [ seg[0] for seg in vdj_aligner_v11.dict2sorteddecreasingitemlist(Vscores_hash,'value')[0:self.numCrudeVCandidates] ]
        ...
        for goodVseg in goodVseglist:
            ...
            currscore = vdj_aligner_v11.scoreVJalign(scores)
            if currscore > bestVscore:
                bestVscore = currscore
                bestVseg = goodVseg
                bestVscoremat = scores
        ...
        v_end_idx = 0   # to ensure it gets defined (for processing j_start_idx below)
        if bestVseg != '':
            Valnref,Valnrefcoords,Valnquery,Valnquerycoords = vdj_aligner_v11.construct_alignment( refseq.IGHV_seqs[bestVseg], query, bestVscoremat, bestVtracemat )
            query,v_end_idx = vdj_aligner_v11.pruneVregion( Valnref, Valnrefcoords, Valnquery, Valnquerycoords, bestVseg, query )
            chain.add_tags('v_end_idx|%d'%v_end_idx)
        ...
        # get numCrudeJCandidates highest scores in Jscores and store their names in descending order
        goodJseglist = [ seg[0] for seg in vdj_aligner_v11.dict2sorteddecreasingitemlist(Jscores_hash,'value')[0:self.numCrudeJCandidates] ]
        ...
        for goodJseg in goodJseglist:
            ...
            # pure python:
            #scores,trace = vdj_aligner_v11.alignNW( refseq.IGHJ_seqs[goodJseg], query )
        ...
        # reconstruct the alignment and chop off J region after the TRP (IMGT)
        if bestJseg != '':
            Jalnref,Jalnrefcoords,Jalnquery,Jalnquerycoords = vdj_aligner_v11.construct_alignment( refseq.IGHJ_seqs[bestJseg], query, bestJscoremat, bestJtracemat )
            (query,j_start_idx) = vdj_aligner_v11.pruneJregion( Jalnref, Jalnrefcoords, Jalnquery, Jalnquerycoords, bestJseg, query )
            chain.add_tags('j_start_idx|%d'%(v_end_idx+j_start_idx))
        ...
        #     # compute hashes from junction sequence using mini seed patterns
        #     queryannot,querykeys = vdj_aligner_v11.seq2kmerannot(query,self.miniseedpatterns)
        #     
        #     t9 = time.time()
        #     
        #     Dscores_hash = {}
        ...
        #     
        #     # get numCrudeDCandidates highest scores in Dscores and store their names in descending order
        #     goodDseglist = [ seg[0] for seg in vdj_aligner_v11.dict2sorteddecreasingitemlist(Dscores_hash,'value')[0:self.numCrudeDCandidates] ]
        #     
        #     if verbose:
        #         print goodDseglist
        #     
        ...
        #         
        #         # pure python:
        #         #scores,trace = vdj_aligner_v11.alignSW( refseq.IGHD_seqs[goodDseg], query )
        #         
        #         currscore = vdj_aligner_v11.scoreDalign(scores)
        #         if currscore > bestDscore:
        #             bestDscore = currscore
        #             bestDseg = goodDseg
        #             bestDscoremat = scores
    ...
    def seq2posstrand(self,seq):        
        seqannot,seqkeys = vdj_aligner_v11.seq2kmerannot(seq,[self.patternPos])
        seqwords = seqkeys[self.patternPos]
        strandid = 1
        if len(self.negset & seqwords) > len(self.posset & seqwords):
            strandid = -1
    ...
    def seqdict2kmerannot(seqdict,patterns):
        ...
        seqlistkeys  = {}
        for seq in seqdict.iteritems():
            seqannot,seqkeys = vdj_aligner_v11.seq2kmerannot(seq[1],patterns)
            seqlistannot[seq[0]] = {}
            seqlistkeys[seq[0]]  = {}
            for pattern in patterns:
                seqlistannot[seq[0]][pattern] = seqannot[pattern]
